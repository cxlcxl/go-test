...
func InitApiRouter() *gin.Engine {
	...
	flowApi := router.Group("/flow/")
	{
		flowApi.GET("exposure", func(context *gin.Context) {
			(&api.Flow{}).HandleFlowExposure(context)
		})
	}
	...
	vApi := router.Group("/api/v1/")
	{
		...
		home := vApi.Group("home/")
		{
			...
			// 3.将以上验证器注册在容器：app/http/validator/common/register_validator/api_register_validator.go  18 行为注册时的键（"HomeNews"）。那么获取的时候就用该键即可从容器获取
			home.GET("news", validatorFactory.Create("HomeNews"))
		}
	}
	return router
}
